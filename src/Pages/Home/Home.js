...
const Home = () => {
  ...
  const [keyMatchedCaptionFound, setKeyMatchedCaptionFound] =
    React.useState(true);
  ...
  const getTranscriptHandler = async () => {
    ...
    if (match && match[2].length === 11) {
      ...
      axios
        .get(
          `https://ytb-api.azurewebsites.net/api/ytb-t01?video_id=${match[2]}`
        )
        .then(async (res) => {
          setLoader(false);
  ...
  const getKeyElements = async (transcript) => {
    return getToken().then((t) => {
      ...
      return axios
        .post(`${baseURL}/v2/analyze/standard/${language}/relevants`, payload, {
          headers: headers,
        })
        .then((res) => {
  ...
  const getKeyNotes = () => {
    ...
    getToken().then((t) => {
      ...
      axios
        .post(`${baseURL}/v2/analyze/standard/${language}/entities`, payload, {
          headers: headers,
        })
        .then((res) => {
          setKnLoader(false);
  ...
  const getSentimentAnalysis = () => {
    ...
    axios
      .get(
        `https://youtube.googleapis.com/youtube/v3/commentThreads?part=snippet&maxResults=100&order=relevance&videoId=${videoId}&key=AIzaSyApX3bSpv8b3y1PEiA29VYI5jh1ZEyd7EQ`
      )
      .then((res) => {
        let comments = "";
        ...
        getToken()
          .then((t) => {
            ...
            axios
              .post(
                `${baseURL}/v2/analyze/standard/${language}/sentiment`,
                payload,
                { headers: headers }
              )
              .then((r) => {
                ...
                let chartData = [
                  _.meanBy(
                    _.filter(sentimentsArray, (o) => o.sentiment < -8),
                    (o) => o.sentiment
                  ),
                  _.meanBy(
                    _.filter(
                      sentimentsArray,
                      (o) => o.sentiment < 0 && o.sentiment >= -8
                    ),
                    (o) => o.sentiment
                  ),
                  _.meanBy(
    ...
    >
      <div className="container transcript" id="getstarted">
        <div className="text-centre">
          ...
          <h1 className="color-light">Enter a youtube link to Summarize</h1>
          <p>
            <br />
            <br />
            <br />
        ...
        <div className="row">
          <div className="col-md-6">
            <div className="inner">
              ...
              <button
                type="submit"
                className="button button-v2"
                onClick={getTranscriptHandler}
              >
                Get Video
              </button>
              {loader ? <Loader /> : ""}
            ...
            <div className="grid-item">
              <header className="App-header">
                {videoId.length === 11 ? (
                  <YoutubeIFrame
                    videoId={videoId}
                    startTimeInSeconds={Math.floor(startTime)}
                  />
                ) : (
                  <div className="dummy" />
                )}
            ...
            {topics.length > 0
              ? topics.map((topic) => (
                  <div style={{ display: "inline-table" }}>
                    <span className="button button-v4 button-sm">#{topic}</span>
                  </div>
                ))
              : ""}
          </div>
            ...
            <div className="row">
              ...
              {vis2 ? (
                <div className="col-md-12">
                  <h2>Key Notes</h2>
                  {knLoader ? <Loader /> : ""}
                  {keyMatchedCaptions.map((c) => (
                    <div style={{ display: "inline-table" }}>
                      <button
                        className="button button-v3 button-sm"
                        onClick={() => setStartTime(c.start)}
                        type="submit"
                      >
                        {c.matchedPhrase} - {c.start}
                      </button>
                    </div>
                  ))}
                  {!keyMatchedCaptionFound
                ...
                <div className="col-md-12">
                  ...
                  <div className={"row"}>
                    ...
                    <div className={"col-md-5"}>
                      ...
                      <p
                        className={
                          "fontsize-lg text-centre color-primary sentiment-score"
                        }
                      >
                        {overallSentiment}
                      </p>
                      <p className={"fontsize-xs text-right"}>
                        -100 Most Negative <br /> 100 Most Positive
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="col-md-12">
                  ...
                  <ol>
                    {quiz.length > 0
                      ? quiz.map((q) => (
                          <li>
                            <h4 className="color-primary fontsize-sm">
                              {q?.sentence}
                            </h4>
                            <ol>
                              {q?.options.map((o) => (
                                <li
                                  onClick={(o) => {
                                    if (
                                      o.target.outerText.toLowerCase() ===
                                      q.answer.toLowerCase()
                                    ) {
                                      document.getElementById(
                                        q.answer
                                      ).innerHTML = "Your Answer is Correct";
                                    } else
                                      document.getElementById(
                                        q.answer
                                      ).innerHTML = "Incorrect Answer";
                                  }}
